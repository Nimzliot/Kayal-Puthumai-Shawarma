...
export async function getAdminOrders(limit = 25) {
  ...
  const { data, error } = await supabase
    .from("orders")
    .select(
      "id, user_id, status, total_amount, subtotal_amount, delivery_charge, tip_amount, eta_minutes, delivery_address, gps_latitude, gps_longitude, notes, created_at, updated_at"
    )
